...
<body class="content-wrapper">
                    ...
                    <table class="text-black mt-1 f-13 b-collapse rightaligned">
                        <tr>
                            ...
                            <td class="heading-table-right">
                                {{ now()->translatedFormat($company->date_format) }}
                            </td>
                        </tr>
    ...
    <table width="100%" class="f-14 b-collapse">
        ...
        @endphp
            <tr class="main-table-items text-black">
                ...
                <td width="60%" class="border-bottom-0">
            {{ $invoice->package->name }}
            @if ($invoice->package->default != 'trial')
                - @lang('superadmin.' . $company->package_type) @if ($invoice->is_addons==1)
                ({{ $invoice->addon_users }} Users Purchased)
                @endif
            @endif
        </td>
                <td align="right" class="border-bottom-0">{{ $invoice->pay_date?->format($global->date_format) }}</td>
                <td align="right" class="border-bottom-0">{{ number_format((float) $invoice->total, 2, '.', '') }}</td>
            </tr>
        @endforeach
        <!-- Table Row End -->
    </table>
    ...
    <table width="100%" class="f-14 b-collapse">
        <tr>
            ...
            <td class="total-box" align="right" width="20%">
                <table width="100%" class="b-collapse">
                    ...
                    <tr align="right" class="text-grey">
                        <td class="subtotal-amt">
                            ...
                            @else
                                ₹
                            @endif{{ number_format((float) $total, 2, '.', '') }}
                        </td>
                    </tr>
                    <!-- Table Row End -->
                    <!-- Table Row Start -->
                    <tr align="right" class="text-grey">
                        <td class="total-amt f-15">
                            ...
                            @else
                                ₹
                            @endif{{ number_format((float) $subTotal, 2, '.', '') }}
                        </td>
                    </tr>
                    <!-- Table Row End -->
                </table>
                ...
                @if ($globalInvoiceSetting->authorised_signatory && $globalInvoiceSetting->authorised_signatory_signature)
                    <td class="" align="right">
                        <img style="height:95px; margin-bottom: -50px; margin-top: 5px;"
                            ...
                            alt="{{ $global->company_name }}" id="logo" /><br>
