...
class ClientSettingController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        $module = Module::where('module_name', 'clients')->first();
        $this->mail->companySmtpSetting = CompanySmtpSetting::where('company_id', '=', company()->id)->where('module_id', $module->id)->first() ?? [];
        $this->mail->companyEmailTemplates = CompanyEmailTemplateSetting::where('company_id', '=', company()->id)->get();
        ...
        case 'client-email-setting':
                $this->view = 'recruit::client-setting.ajax.email-setting';
                break;
        default:
        ...
        $this->activeTab = $tab ?: 'client-email-setting';

        if (request()->ajax()) {
            $html = view($this->view, $this->data)->render();
            return Reply::dataOnly(['status' => 'success', 'html' => $html, 'title' => $this->pageTitle, 'activeTab' => $this->activeTab]);
        }
        return view('recruit::client-setting.index', $this->data);
    }
    ...
    public function addCompanySmtpSetting(Request $request) {
        ...
        // Update or create the SMTP settings for the company

        $smtpSetting = CompanySmtpSetting::updateOrCreate(
            ['module_id' => $module->id], // Condition to check if the setting exists
            [
